...
interface DropdownSeriesOptionProps {
  seriesItem: DicomSeries;
  isSelected: boolean;
  onSelect: (seriesId: string) => void;
}

const DropdownSeriesOption = memo(function DropdownSeriesOption({
  seriesItem,
  isSelected,
  onSelect
}: DropdownSeriesOptionProps) {
  const handleClick = useCallback(() => {
    onSelect(seriesItem.id);
  }, [onSelect, seriesItem.id]);
  ...
  return (
    ...
    >
      <div
        className={`w-8 h-8 rounded-md border flex items-center justify-center text-xs font-semibold flex-shrink-0 overflow-hidden ${
          isSelected
...
DropdownSeriesOption.displayName = 'DropdownSeriesOption';
...
export default function MobileSeriesDropdown({
  ...
  const handleSelect = useCallback((seriesId: string) => {
  ...
  return (
    <div className="relative">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            ...
            <div className="text-xs text-neutral-400">
              {selectedSeries
                ? `${selectedSeries.instanceCount} images`
      ...
      {isOpen && (
        <div
          className="absolute top-full left-0 right-0 z-50 mt-2 bg-neutral-800/95 backdrop-blur-sm border border-neutral-700 rounded-lg shadow-xl max-h-60 overflow-y-auto"
          role="listbox"
        >
          {series.map(seriesItem => (
            <DropdownSeriesOption
              key={seriesItem.id}
              seriesItem={seriesItem}
              isSelected={selectedSeriesId === seriesItem.id}
